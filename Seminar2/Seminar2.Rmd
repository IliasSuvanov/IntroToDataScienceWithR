...
### Some materials are taken from the online course on coursera "Statistics with R"
...
<ul>
<li>Print numbers from 150 to 170 in a loop.</li>
<li>Print in a loop "This is year ...", where dots replace with years from 2015 to 2020.</li>
</ul>
...
library(statsr)
data("arbuthnot")
if(arbuthnot$boys[1]>arbuthnot$girls[1]){
   status<- "up"} else {
   status <'down'}
```
...
if(arbuthnot$boys[1]>arbuthnot$girls[1]){
  status <- 'up'
}else if(arbuthnot$boys[1]>arbuthnot$girls[1]){
  status <- 'down'
} else {
  status <- 'flat'
}
...
```{r}
arbuthnot$status <- ifelse(arbuthnot$boys > arbuthnot$girls, 'up','down')
```
...
![](/apple.png)
Function is just a word that encapsulates sets of instructions. When you execute your function, you will basically execute your sets of instructions.


As a simple introduction, let’s define a function named oddcount(), whose
